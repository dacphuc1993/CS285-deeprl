...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        indices = [index for index  in range(len(rewards))]

        # 2) create a list where the entry at each index (t') is gamma^(t')
        discounts = [self.gamma ** index for index in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [discount * rewards[i] for i, discount in enumerate(discounts)]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = [index for index  in range(start_time_index, len(rewards))]

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [self.gamma ** (index - start_time_index)for index in indices]
            ...
            discounted_rtg = [discount * rewards[i] for i, discount in enumerate(discounts)]
