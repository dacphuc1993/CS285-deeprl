...
class BootstrappedContinuousCritic(BaseCritic):
    ...
    def forward(self, ob):
        ...
        # DONE
        prediction = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:ob})
        return prediction
    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if i % self.num_grad_steps_per_target_update == 0:
                ...
                target_q_value = re_n + self.gamma * v_sp

            _ , loss = self.sess.run([self.critic_update_op, self.critic_loss], 
                                    feed_dict={self.sy_ob_no: ob_no, self.sy_target_n:target_q_value})
