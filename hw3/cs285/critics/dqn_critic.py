...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            idx = tf.stack([tf.range(tf.shape(self.obs_t_ph)[0]), tf.cast(tf.argmax(q_tp1_values, axis=1), tf.int32)], axis=1)
            q_tp1 = tf.gather_nd(q_tp1_values, idx)
            # q_tp1 = q_func(self.obs_tp1_ph, temp_ac, scope='q_func', reuse=True)
        else:
        ...
        # TODO these variables should contain all of the 
